...
{
    // ENHANCEMENT: k-means++ seeks to choose better starting clusters. 
    // (http://en.wikipedia.org/wiki/K-means_clustering)
    // ENHANCEMENT: The filtering algorithm uses kd-trees to speed up each 
    // k-means step[9]. (http://en.wikipedia.org/wiki/K-means_clustering)
    // ENHANCEMENT: Euclidean squared distance metric is equivalent for k-means 
    // and faster than euclidean: http://goo.gl/wqI6Qe

    /// <summary>
    /// An ISpeciationStrategy that speciates genomes using the k-means 
    /// clustering method. k-means requires a distance metric and as such this 
    /// class requires am IDistanceMetric to be provided at construction time. 
    /// Different distance metrics can be used including NeatDistanceMetric which is 
    /// equivalent to the metric used in the standard NEAT method albeit with a 
    /// different clustering/speciation algorithm (Standard NEAT does not use k-means).
    /// </summary>
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        /// <summary>
        /// Speciates the genomes in genomeList into the number of species 
        /// specified by specieCount and returns a newly constructed list of 
        /// Specie objects containing the speciated genomes.
        /// </summary>
        public IList<Specie<TGenome>> InitializeSpeciation(IList<TGenome> genomeList, 
                                                           int specieCount)
        {
            // Create empty specieList.
            // Use an initial specieList capacity that will limit the need for 
            // memory reallocation but that isn't too wasteful of memory.
        ...
        /// <summary>
        /// Perform the main k-means loop until no genome reallocations occur or
        /// some maximum number of loops has been performed. Theoretically a small 
        /// number of reallocations may occur for a great many loops therefore 
        /// we require the additional max loops threshold exit strategy - the 
        /// clusters should be pretty stable and well defined after a few loops 
        /// even if the the algorithm hasn't converged completely.
        /// </summary>
        private void SpeciateUntilConvergence(IList<TGenome> genomeList, 
                                              IList<Specie<TGenome>> specieList)
        {
            List<Specie<TGenome>> emptySpecieList = new List<Specie<TGenome>>();
            int specieCount = specieList.Count;

            // Array of flags that indicate if a specie was modified 
            // (had genomes allocated to and/or from it).
            ...
            for (int loops = 0; loops < __MAX_KMEANS_LOOPS; loops++)
            {
                ...
                // Loop over genomes. For each one find the specie it is closest 
                // to; if it is not the specie it is currently in then reallocate it.
                foreach (TGenome genome in genomeList)
                {
                    ...
                    if (genome.SpecieIdx != closestSpecie.Idx) 
                    {
                        ...
                        // Add the genome to its new specie and set its speciesIdx 
                        // accordingly. For now we leave the genome in its original 
                        // species; It's more efficient to determine all reallocations 
                        // and then remove reallocated genomes from their origin 
                        // specie all together; This is because we can shuffle 
                        // down the remaining genomes in a specie to fill the 
                        // gaps made by the removed genomes - and do so in one 
                        // round of shuffling instead of shuffling to fill a gap
                        // on each remove.
                        closestSpecie.GenomeList.Add(genome);
                        genome.SpecieIdx = closestSpecie.Idx;
                        reallocations++;
                ...
                for (int i = 0; i < specieCount; i++)
                {
                    ...
                    // Remove the genomes that have been allocated to other other 
                    // species. We fill the resulting gaps by shuffling down the 
                    // remaining genomes.
                    Specie<TGenome> specie = specieList[i];
                    ...
                    // Track empty species. We will allocate genomes to them 
                    // after this loop. This is necessary as some distance 
                    // metrics can result in empty species occuring.
                    if (0 == specie.GenomeList.Count) {
                        emptySpecieList.Add(specie);
                    }
                    else {
                        // Recalc the specie centroid now that it contains a 
                        // different set of genomes.
                        specie.Centroid = CalculateSpecieCentroid(specie);
                    }
                }

                // Check for empty species. We need to reallocate some genomes 
                // into the empty specieList to maintain the required number of species.
                if (0 != emptySpecieList.Count)
                {
                    // We find the genomes in the population as a whole that are 
                    // furthest from their containing specie's centroid genome - 
                    // we call these outlier genomes. We then move these genomes
                    // into the empty species to act as the sole member and
                    // centroid of those speciea; These act as specie seeds for 
                    //vthe next k-means loop.
                    TGenome[] genomeByDistanceArr = 
                            GetGenomesByDistanceFromSpecie(genomeList, specieList);

                    // Reallocate each of the outlier genomes from their current
                    // specie to an empty specie.
                    int emptySpecieCount = emptySpecieList.Count;
                    int outlierIdx = 0;
                    for (int i = 0; i < emptySpecieCount; i++)
                    {
                        // Find the next outlier genome that can be re-allocated. 
                        // Skip genomes that are the only member of a specie - 
                        // that would just create another empty specie.
                        TGenome genome;
                        ...
                        while (sourceSpecie.GenomeList.Count == 1 && 
                               outlierIdx < genomeByDistanceArr.Length);

                        if (outlierIdx == genomeByDistanceArr.Length)
                        {   // Theoretically impossible. We do the test so that 
                            // we get an easy to trace error message if it does happen.
                        ...
                        // Get ref to the empty specie and register both source 
                        // and target specie with specieModArr.
                        Specie<TGenome> emptySpecie = emptySpecieList[i];
                        specieModArr[emptySpecie.Idx] = true;
                        specieModArr[sourceSpecie.Idx] = true;

                        // Reallocate the genome. Here we do the remove operation 
                        // right away; We aren't expecting to deal with many empty
                        // species, usually it will be one or two at most; Any 
                        // more and there's probably something wrong with the distance
                        // metric, e.g. maybe it doesn't satisfy the triangle 
                        // inequality (see wikipedia). Another reason to remove 
                        // right is to eliminate the possibility of removing 
                        // multiple outlier genomes from the same specie and 
                        // potentially leaving it empty; The test in the do-while
                        // loop above only takes genomes from currently non-empty species.
                    ...
                    for (int i = 0; i < specieCount; i++) {
                        if(specieModArr[i]) 
                        {   // Reset flag while we're here. Do this first to help 
                            // maintain CPU cache coherency (we just tested it).
                            specieModArr[i] = false;
                            specieList[i].Centroid = CalculateSpecieCentroid(specieList[i]);
                        }
                    }

                    // Clear emptySpecieList after using it. Otherwise we are 
                    // holding old references and thus creating work for the 
                    // garbage collector.
                    emptySpecieList.Clear();
                }

                // Exit the loop if no genome reallocations have occured. 
                // The species are stable, speciation is completed.
                if (0 == reallocations) {
                    break;
                }
            }
        ...
        /// </summary>
        private TGenome[] GetGenomesByDistanceFromSpecie(IList<TGenome> genomeList, 
                                                         IList<Specie<TGenome>> specieList)
        {
            // Build a list of all genomes paired with their distance from their centriod.
            int genomeCount = genomeList.Count;
            GenomeDistancePair<TGenome>[] genomeDistanceArr = 
                    new GenomeDistancePair<TGenome>[genomeCount];
            for (int i = 0; i < genomeCount; i++)
            {
                TGenome genome = genomeList[i];
                double distance = 
                        _distanceMetric.MeasureDistance(genome.Position, 
                                                        specieList[genome.SpecieIdx].Centroid);
