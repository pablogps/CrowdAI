...
{
    /// <summary>
    /// Decodes NeatGenome's into specific network instances.
    /// </summary>
    public class NeatGenomeDecoder : IGenomeDecoder<NeatGenome,IBlackBox>
    {
        ...
        {
            if (activationScheme.Esp)
            {
                return DecodeToEspCyclicNetwork;
            }
        ...
        private EspCyclicNetwork DecodeToEspCyclicNetwork(NeatGenome genome)
        {
            return EspCyclicNetworkFactory.CreateEspCyclicNetwork(genome, _activationScheme);
        }
