...
namespace SharpNeat.Phenomes.NeuralNets
{
	...
	/// Remember that in the phenome kind FastAcyclicNetwork networks are organized by their depth (distance in signal 
	/// transmission from the input neurons) so we expect nodes in the same layer to be contiguous, so storing the 
	/// last node and connection is enough.
    /// </summary>
