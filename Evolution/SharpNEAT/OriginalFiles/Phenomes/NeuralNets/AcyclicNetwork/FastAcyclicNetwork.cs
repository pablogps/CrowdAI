...
namespace SharpNeat.Phenomes.NeuralNets
{
    ...
    //=== Fixed data. Network structure and activation functions/data.
        ...
        /// <summary>
		/// Array of connections. FastConnection represents a single connection: weigth and source/target neurons.
        /// </summary>
    ...
    //=== Misc.
        ...
        {   
            ...
            for(int layerIdx = 1; layerIdx < _layerInfoArr.Length; layerIdx++)
            {
				...
				// Propagates the previous layer's neurons' signal through their connections to deeper layers.
				// Note a neuron from layer i can be connected to any layer of depth j > i, not only to i + 1!
                for(; conIdx < layerInfo._endConnectionIdx; conIdx++) {
                    _activationArr[_connectionArr[conIdx]._tgtNeuronIdx] += 
							_activationArr[_connectionArr[conIdx]._srcNeuronIdx] * _connectionArr[conIdx]._weight;
                }

				// Activate current layer's nodes (we are sure these will not receive any more input).
