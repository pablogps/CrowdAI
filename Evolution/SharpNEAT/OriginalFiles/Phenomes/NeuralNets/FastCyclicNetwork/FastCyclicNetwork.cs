...
{
    // not-TODO: Create a version of this class that uses single precision signals 
    // for extra performance. NOTE: Actually, on my system (this is Pablo writing)
    // float calculations for activation functions take twice as long! Still really
    // fast though (a million evaluations took 0.035 vs 0.070 seconds).
    // TODO: Reconsider algorithm. Even better/faster way?

    /// <summary>
    /// A neural network class that represents a network with recurrent
    /// (cyclic) connections. 
    /// 
    /// This is a much faster implementation of CyclicNetwork. The speedup is
    /// approximately 5x depending on hardware and CLR platform, see 
    /// http://sharpneat.sourceforge.net/network_optimization.html for detailed info.
    /// 
    /// The speedup is achieved by compactly storing all required data in arrays
    /// and in a way that maximizes in-order memory accesses; this allows us to
    /// maximize use of CPU caches. In contrast the CyclicNetwork class 
    /// represents the network directly, that is, as a network of neuron/node
    /// objects. This has additional overhead such as the standard data
    /// associated with each object in dotNet which results in less efficient
    /// packing of the true neural net data in memory, which in turns results in
    /// less efficient use of CPU memory caches. Finally, representing the
    /// network directly as a graph of connected nodes is not conducive to 
    /// writing code with in-order memory accesses.
    /// 
    /// Algorithm Overview.
    /// 1) Loop connections. Each connection gets its input signal from its
    /// source neuron, applies its weight and stores its output value.
    /// Connections are ordered by source neuron index, thus all memory accesses
    /// here are sequential/in-order.
    /// 
    /// 2) Loop connections (again). Each connection adds its output value to
    /// its target neuron, thus each neuron accumulates or 'collects' its input
    /// signal in its pre-activation variable. Because connections are sorted by
    /// source neuron index and not target index, this loop generates out-of
    /// order memory accesses, but is the only loop to do so.
    /// 
    /// 3) Loop neurons. Pass each neuron's pre-activation signal through the
    /// activation function and set its post-activation signal value. 
    /// 
    ...
    public class FastCyclicNetwork : IBlackBox
    {
        ...
        /// <summary>
        /// Constructs a FastCyclicNetwork with the provided pre-built
        /// FastConnection array and associated data.
        /// </summary>
                                 ...
                                 int timestepsPerActivation)
        {
            ...
            // arrays for IBlackBox. Offset is 1 to skip bias neuron (The value
            // at index 1 is the first black box input).
            _inputSignalArrayWrapper = new SignalArray(_postActivationArray, 1,
                                                       inputNeuronCount);

            // Offset to skip bias and input neurons. Output neurons follow
            // input neurons in the arrays.
        ...
        public virtual void Activate()
        {
            ...
            for (int i = 0; i < _timestepsPerActivation; i++)
            {
                ...
                // Loop the neurons. Pass each neuron's pre-activation signals
                // through its activation function and store the resulting
                // post-activation signal. Skip over bias and input neurons as
                // these have no incoming connections and therefore have fixed
                ...
                for (int j = _inputAndBiasNeuronCount; j < _preActivationArray.Length; j++)
                {
                    _postActivationArray[j] = _neuronActivationFnArray[j].Calculate(_preActivationArray[j], 
																					_neuronAuxArgsArray[j]);                    
                    // Take the opportunity to reset the pre-activation signal
                    // array in preperation for the next activation loop.
        ...
        {
            // TODO: Avoid resetting if network state hasn't changed since
            // construction or previous reset.
